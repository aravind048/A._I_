...
from agent import get_stem_answer
import pyttsx3
import re
import threading
...
audio = None  # PyAudio instance (the audio system itself)
stream = None  # actual microphone input stream
ws_app = None  # WebSocket connection to AssemblyAI
audio_thread = None  # thread that handles microphone streaming in parallel
...
def on_open(ws):
    ...
    def stream_audio():
        ...
        if not stream:
            print("❌ Audio stream is not initialized. Exiting audio thread.")
            return
...
def on_message(ws, message):
    try:
        ...
        if msg_type == "Begin":
            ...
            # .get('data to be fetched', 'default value')
            ...
            if formatted and transcript:
                ...
                answer = get_stem_answer(transcript)
                cleaned = postprocess(answer)
                print(f"📘 STEM Tutor: {cleaned}")
                speak_text(cleaned)
...
def save_wav_file():
    ...
    try:
        ...
        print(
            f"Duration: {len(recorded_frames) * FRAMES_PER_BUFFER / SAMPLE_RATE:.2f} seconds")
...
def postprocess(text: str) -> str:
    # Remove any assistant tokens or hallucinated questions
    text = re.sub(r"\[/?(INST|ASSIST)\]", "", text, flags=re.IGNORECASE)
    text = re.sub(r"(?i)(who|what|where|when|why|how) is .*?\?", "", text)
    ...
    try:
        ...
        if ws_app and ws_app.sock and ws_app.sock.connected:
            try:
                ...
                print(
                    f"Sending termination message: {json.dumps(terminate_message)}")
